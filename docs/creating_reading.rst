...
models using the parameter values from this paper using::
    ...
    # HI Model of Liszt & Burton (1980) rescaled to model galcen_distance
    >>> hi_cube = EmissionCube.create_LB80()
...
Similarly, you can create the H-Alpha model using::
    ...
    # H-Alpha model of Krishnarao et al. (2020) including dust reddening
    >>> ha_cube = EmissionCube.create_DK20()
...
some parameters on the tilt angles using the `model = True` keyword::
    ...
    >>> hi_cube = EmissionCube.read("LisztBurton1980_HI_Model.fits", model = True)
    >>> print("Tilt Angles of the Liszt & Burton (1980) HI Model are:")
    >>> print("Alpha = {0:.2f}, Beta = {1:.2f}, Theta = {2:.2f}".format(hi_cube.alpha, 
                                                                        hi_cube.beta, 
                                                                        hi_cube.theta))
    Tilt Angles of the Liszt & Burton (1980) HI Model are:
...
`~modspectra.cube.EmissionCube.create_DK20` and `~modspectra.cube.EmissionCube.create_LB80`. 
A full list of the keywords can be found in the :class:`~modspectra.cube.EmissionCube` class page::
    ...
    >>> ha_cube = EmissionCube.create_DK20(resolution = resolution, redden = False)

    # Compute HI cube with larger scale height and higher resolution
    # Use memory mapping via dask
    resolution = (400,400,400)
    Hz = 0.2 # kpc
    >>> hi_cube = EmissionCube.create_LB80(resolution = resolution, Hz = Hz,  
                                           memmap = True)
